...
- (IBAction)showHideQPADrawer:(id)sender
{
    ...
    if (drawerOpen) {
        ...
        y = self.view.frame.size.height + 64 - qpaDrawer.frame.size.height;
        img = @"toolbar2.png";
...
- (void)viewDidLoad
{
    ...
    // QPA Drawer
    qpaDrawer = [[QPADrawer alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height-24, 320, 194)];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (currentSem == NULL) {
        ...
        [UIView animateWithDuration:1.0 delay:1.0 options:UIViewAnimationCurveEaseIn animations:^ {
            addTip1.alpha = 1.0;
        } completion:^(BOOL finished) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row != 0) {
        ...
        editView.center = CGPointMake(160, self.view.frame.size.height/2+40);
